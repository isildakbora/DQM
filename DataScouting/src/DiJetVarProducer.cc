...
DiJetVarProducer::DiJetVarProducer(const edm::ParameterSet& conf) :
    ...
    useWideJets_(conf.getParameter<bool>("useWideJets")),
...
DiJetVarProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    if( calojets_handle->size() >=2 ) {
        ...
        TLorentzVector jet1, jet2,  FirstLeadingJet, SecondLeadingJet;
        ...
        for(reco::CaloJetCollection::const_iterator it = calojets_handle->begin(); it != calojets_handle->end(); ++it) {
            ...
            if( (i==0 || i==1) ) {
                ...
                if(it->energyFractionHadronic()>maxHADfraction_ || it->emEnergyFraction()>maxEMfraction_) {
                    pass_JetIDtwojets=false;
                }

                if(i == 0) FirstLeadingJet.SetPtEtaPhiM(correctedJet.pt(),correctedJet.eta(),correctedJet.phi(),correctedJet.mass());
                else       SecondLeadingJet.SetPtEtaPhiM(correctedJet.pt(),correctedJet.eta(),correctedJet.phi(),correctedJet.mass());
